...
class ManagedBleakDeviceTakeOver(ManagedBleakDevice):
    """
    ...
    def __init__(self):
        super(ManagedBleakDeviceTakeOver, self).__init__()
    ...
    async def change_address(self,
                       address: Optional[Union[BLEDevice, str]] = None) -> bool:
        if self.address not in ('', None):
            raise DE1ValueError(
                f"{self.__class__.__name__} has a set-once address. "
                f"Create a new instance to change from {self.address} "
                "and consider calling take_over_from(old_device)"
            )
        return await super(ManagedBleakDeviceTakeOver, self
                           ).change_address(address=address)

    # TODO: 3.11 has Self annotation PEP 673
    def take_over_from(self, old_device: 'ManagedBleakDeviceTakeOver') \
            -> 'ManagedBleakDeviceTakeOver':
        """
        ...
        Typical use:
            old_ref = new_device.take_over_from(old_ref)
        """
        local_logger = self.logger.getChild('TakeOverFrom')

        if old_device is self:
            local_logger.error("take_over_from same device, ignoring request")
            pass

        elif old_device is None:
            local_logger.error("take_over_from None, ignoring request")
            pass

        else:
            for attr in self._attrs_to_take_over:
                try:
                    old_attr = getattr(old_device, attr)
                    try:
                    ...
                    local_logger.error(
                        f"old_device {old_device} missing {attr} attribute {e}")
