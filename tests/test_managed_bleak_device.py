...
from pyDE1.bledev.managed_bleak_device import (
    cq_to_cs, ManagedBleakDevice, ManagedBleakDeviceTakeOver, cq_to_das
)
...
class SwappingMBD(ManagedBleakDeviceTakeOver):
    """
    Implements ManagedBleakDevice but with a fixed address,
    set at initialization time. Useful for Scale where the
    class is determined by the BLE signature.

    Provides take_over_from(old_instance) which helps enable
    replacement with a different instance.
    """
    ...
    def __init__(self, logger: logging.Logger = None):
        ...
        self.logger = logger
        super(SwappingMBD, self).__init__()
...
class GenericSwappingMBD (SwappingMBD):
    pass


class Type1SwappingMBD (SwappingMBD):

    def __init__(self):
        super(Type1SwappingMBD, self).__init__()
        self._attrs_to_take_over.append('_this_does_not_exist')


@pytest.mark.asyncio
async def test_swapping_mbd(mock_bleak_backend,
                            mock_send_to_outbound_pipes,
                            caplog):
    ...
    # tlname = inspect.currentframe().f_code.co_name
    tlname = '===>'
    test_logger = logging.getLogger(tlname)
    caplog.set_level(logging.DEBUG, tlname)
    caplog.set_level(logging.DEBUG, 'Notify.Payload')


    mbd = GenericSwappingMBD(logger=logger)
    await mbd._event_connectivity.subscribe(log_payload)
    ...
    assert mbd.address == ''
    test_logger.debug("Setting address")
    use_address = '11:22:33:44:55:66'
    await mbd.change_address(use_address)
    assert mbd.address == use_address
    ...
    test_logger.debug("Starting capture")
    await mbd.capture(1.0)
    assert mbd.connectivity_state in (ConnectivityState.CONNECTED,
                                      ConnectivityState.READY)
    await asyncio.wait_for(mbd.event_ready.wait(), 0.100)
    ...
    test_logger.debug("Starting release")
    await mbd.release(1.0)
    test_logger.debug("Done")
    ...
    test_logger.debug("Starting change of address")
    assert isinstance(mbd, GenericSwappingMBD)
    next_address = 'aa:bb:cc:dd:ee:ff'
    with pytest.raises(ValueError) as exec_info:
        await mbd.change_address(next_address)
        assert "Create a new instance to change from" in str(
                exec_info.value)
    assert not mbd.is_connected
    assert     mbd.is_released
    assert not mbd.is_captured
    assert not mbd.is_ready
    assert mbd.connectivity_state == ConnectivityState.DISCONNECTED
    await asyncio.sleep(0.100)
    assert mock_send_to_outbound_pipes.connectivity_sent == []
    mock_send_to_outbound_pipes.connectivity_sent = []

    test_logger.debug("Now try replacing")
    new_mbd = Type1SwappingMBD()
    await new_mbd.change_address(next_address)
    assert not new_mbd.is_connected
    assert not new_mbd.is_released
    assert not new_mbd.is_captured
    assert not new_mbd.is_ready
    assert new_mbd.connectivity_state == ConnectivityState.INITIAL

    check_sent = list(zip(
        ['INITIAL', 'DISCONNECTING', 'DISCONNECTING', 'DISCONNECTING',
         'DISCONNECTED', 'INITIAL'],
        ['']*5 + [next_address]))
    await asyncio.sleep(0.100)
    assert mock_send_to_outbound_pipes.connectivity_sent == check_sent
    ...
    test_logger.debug("Starting take over")

    assert not hasattr(mbd, '_this_does_not_exist')

    mbd = new_mbd.take_over_from(mbd)
    assert old_id != new_id
    assert id(mbd) == new_id
    assert isinstance(mbd, Type1SwappingMBD)
    assert not hasattr(mbd, '_this_does_not_exist')
    found_the_record = 0
    id_as_hex = f"{new_id:x}"
    for rec in caplog.records:
        if (rec.levelno == logging.ERROR
                and 'missing _this_does_not_exist attribute' in rec.message):
            found_the_record += 1
        if rec.name == 'Notify.Payload':
            if next_address in rec.message:
                assert 'Type1SwappingMBD' in rec.message
                assert id_as_hex in rec.message
    assert found_the_record == 1


    test_logger.debug("Starting new capture")
    await mbd.capture(1.0)
    assert mbd.connectivity_state in (ConnectivityState.CONNECTED,
                                      ConnectivityState.READY)
    await asyncio.wait_for(mbd.event_ready.wait(), 0.100)
